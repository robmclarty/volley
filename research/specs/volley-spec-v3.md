...
- **§4 (builder tool surface).** The tool set is enumerated but the exact schemas, caps, and error semantics want the same rigor `src/critic/tools.ts` already has. Reuse `contain()` where path-scoping is the boundary; be explicit about where `bash` makes path-scoping meaningless and the container/worktree becomes the only boundary. **Update:** a tool-strategy research pass ([../local-builder-tool-strategy.md](../local-builder-tool-strategy.md), sourced synthesis of academic/industry/open-source prior art) reframes §4 around *transport* (native tool-calling vs. text-protocol), not just the tool list — and reflects **fascicle 0.8.13**, which shipped the tool-call *salvage* and per-step *clamp* options this analysis called for. Fold those findings first.
...
### Encoding is the real §4/§6 decision (verified against fascicle 0.8.13)

The tool *list* above is the easy part; the load-bearing choice is the **transport** — how a local model's actions actually reach volley. There are two, and they drive the identical `builder_tools` set and termination story (§5); only the wire format differs:

- **Native function-calling** — the path this section assumes, and the one the local critic already rides via `generate({ tools })`. fascicle serializes `tools: Tool[]` through ai-sdk to the runtime's own tool parser. Two knobs make it survivable on local runtimes, both confirmed in the installed source: `tool_error_policy: 'feed_back'` returns a throwing tool's failure to the model as a normal result (§12 Q3), and **fascicle 0.8.13's `tool_call_repair_attempts`** salvages a call the model emitted as assistant *text* — the dominant Ollama/Qwen failure — in Hermes (`<tool_call>{…}</tool_call>`), bare/`json`-fenced, or Qwen3-Coder XML form, accepted only when the name resolves and the args validate against the tool's `input_schema` (so ordinary JSON-in-prose never false-positives). `max_tool_calls_per_step: 1` neutralizes the parallel-call failure. With these set, a mis-parsed call is *recovered and marked `salvaged`*, not a silent loop-ender — so the native path is a viable default, not a dead end. The builder is still far more exposed than the read-only critic (dozens of `bash`/edit turns), so the salvage rate is a health signal to watch, not ignore.
- **Text-protocol** — mini-swe-agent's approach: enumerate the tools in the system prompt, have the model emit a fenced action, and let *volley* parse and dispatch it, removing the runtime tool parser from the path entirely. Every open-source agent that targets local models (mini-swe-agent, Aider, qwen-agent, pre-native Cline) converges on this. It needs no native tool-calling to work, and can be prototyped in volley today by calling `generate()` with **no** `tools` (plain completion) and running the tool loop harness-side. With 0.8.13's salvage in place this drops from co-equal option to **deeper fallback** — reached only when native+salvage still can't recover calls on the pinned runtime.

Fable: specify the builder's encoding as a **layered default, not a coin-flip.** Tier 1 (recommended): native `tools` + `tool_call_repair_attempts > 0` + `max_tool_calls_per_step: 1` + `tool_error_policy: 'feed_back'` — this alone should carry a well-pinned runtime. Tier 2 (deeper fallback): the text-protocol loop, entered only if a preflight canary (§10) or a run's salvage-rate telemetry shows native+salvage still failing. The `Tool` shape from `src/critic/tools.ts` is reused verbatim in either tier — native mode has fascicle call `execute` (salvaging text-emitted calls first); text mode has volley parse and call it. Salvage is observable (`salvaged`/`salvaged_format` on the `ToolCallRecord`, a `tool_call_salvaged` trajectory event), so the salvage rate is a first-class run-health metric and a §8 datum.
...
> - **`finish` is transport-agnostic (verified against fascicle 0.8.13).** The LOCKED "explicit `finish`" is a *signal*, realized as a `finish` **tool** in native mode and a **sentinel line** (e.g. a `FINISH` first-line marker) in text-protocol mode (§4). Both satisfy "both `max_steps` and explicit `finish`." `finish_reason: 'max_steps'` is a real value in the installed `FinishReason` union, so the `max_steps` backstop is surfaced natively — no volley bookkeeping needed to detect it.
> - **Tool-call recovery now exists in the substrate (fascicle 0.8.13).** The two *older* knobs still do not cover a mis-emitted call — `schema_repair_attempts` repairs the final **output schema**, `tool_error_policy` handles a tool whose `execute` **throws** — so before 0.8.13 a runtime that mis-serialized a tool def and made the model emit the call as prose ended the loop on a plain `finish_reason: 'stop'`. **0.8.13's `tool_call_repair_attempts` closes exactly that gap:** it salvages the text-emitted call (Hermes / bare-or-`json`-fenced / Qwen3-Coder XML), gated on name-resolves-and-args-validate, runs it through the normal execute path, and marks it `salvaged`. Set `tool_call_repair_attempts > 0` and `max_tool_calls_per_step: 1` on the builder loop; the §4 text-protocol transport is the deeper fallback for what salvage can't recover. §10 reflects this.
...
- **Qwen's own docs do not guarantee protocol adherence** even with correct templates — parse defensively and repair. **This is now a config flag (fascicle 0.8.13):** `tool_call_repair_attempts` performs exactly this defensive parse-and-repair — it salvages a Qwen tool call emitted as assistant text in Hermes or Qwen3-Coder-XML form (its `salvaged_format` values are `hermes` / `json` / `qwen_xml`), gated on schema validation. It is the substrate answer to this section's "parse defensively and repair"; the older `schema_repair_attempts` / `tool_error_policy` knobs do **not** cover it (they repair the output schema and a throwing `execute` respectively — see the §5 note). Set it for the builder; keep the §4 text-protocol transport as the deeper fallback. Full prior-art synthesis: [../local-builder-tool-strategy.md](../local-builder-tool-strategy.md).
...
| Local model emits a tool call as assistant text (mis-serialized def) | **Native mode (fascicle 0.8.13):** with `tool_call_repair_attempts > 0` the call is *salvaged* from Hermes/`json`/Qwen3-Coder-XML text, validated against the tool schema, executed normally, and marked `salvaged` (a `tool_call_salvaged` event fires). Only a format salvage can't parse falls through. **Text mode:** an unparseable fence is re-prompted by volley (mini-swe-agent-style). Either way, only a *persistent* failure — not a one-off — is a builder error (exit 3). |
| Local model emits multiple/parallel tool calls a runtime mishandles | `max_tool_calls_per_step: 1` executes the first and drops the rest for that turn (each dropped call surfaces a `dropped_max_tool_calls_per_step` record + `tool_calls_dropped` event); the model re-issues next turn. Not an error. |
| Native + salvage still can't recover calls on the pinned runtime | The `--dry-run` canary (or a high salvage-failure rate mid-run) routes the builder to the text-protocol encoding (§4); the transport used is recorded in the summary as a §8 confound. |
...
**Substrate update — fascicle 0.8.13.** The engine now ships the two options §4–§6 rely on: `tool_call_repair_attempts` (salvage a text-emitted tool call; sets `salvaged`/`salvaged_format` on the record, emits `tool_call_salvaged`) and `max_tool_calls_per_step` (clamp parallel calls; emits `tool_calls_dropped`). Both are opt-in and default-inert, and the `claude_cli` provider records `option_ignored` for both — so they touch **only** the local builder path and the all-Claude path is byte-for-byte unaffected (a point *for* §8 fairness, not a confound). volley's dependency range (`^0.8.12`) already admits 0.8.13; a `pnpm update fascicle` is needed to pull it (node_modules currently resolves 0.8.12). The two builder seams above are unchanged — `run_builder`'s local branch just sets `tool_error_policy: 'feed_back'`, `tool_call_repair_attempts > 0`, and `max_tool_calls_per_step: 1` on its `generate` call.
...
3. **§4 `tool_error_policy` / tool-call recovery.** Does a non-zero `bash` exit throw or return? (Recommend: return.) **Resolved (fascicle 0.8.13):** on the builder loop set `tool_error_policy: 'feed_back'` (returns a throwing tool's failure to the model), `tool_call_repair_attempts > 0` (salvages a text-emitted call — the mis-parsed-call case the older knobs missed), and `max_tool_calls_per_step: 1` (parallel-call safety). The same `feed_back` stance applies to `fetch` HTTP errors.
...
10. **Fresh context vs. the inner loop.** v2's fresh-context-per-iteration is about the *outer* loop. Confirm the *inner* local-builder tool loop's step history stays within the single `generate` call and is discarded between iterations, preserving the v2 hygiene guarantee. (Holds for native mode, where the inner loop *is* one `generate` call. If the text-protocol transport prototypes in volley as its own per-turn `generate` calls, the outer-loop hygiene is unchanged but the "one `generate` = one builder turn" framing is temporarily broken; it is restored once the transport graduates into fascicle's loop.)
11. **§4/§6 builder transport (new).** With 0.8.13's salvage in place, the canary decides **Tier 1** (native `tools` + `tool_call_repair_attempts`) vs **Tier 2** (text-protocol), not native-vs-nothing. What exactly is the canary (define one trivial tool, ask the pinned model to call it, assert a call is produced *or salvaged*)? Does the summary record both the transport used *and* the per-run salvage rate (both §8 data)? Prior art and the full argument: [../local-builder-tool-strategy.md](../local-builder-tool-strategy.md).
