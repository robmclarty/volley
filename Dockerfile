# volley sandbox image: the hardened container the *whole*
# volley builder runs inside, not just its `bash` commands. It carries
...
# <hardened flags> <image> <volley args>` (the invocation spec in
...
# applied on `docker run` by the invocation spec, not baked here. The
...
# Build volley into the image: the whole builder loop runs in here, so the
...
# The containment marker: volley *detects* it is running inside its sandbox
...
# time (a named volume under the read-only rootfs), so it is not created here.
WORKDIR /workspace
USER node

# The entrypoint runs volley: `docker run <image> --prompt … --workspace
